...
import { frPageOrder, frPages, type FrPageConfig } from "@/content/fr/landingPages";
...
export default function FrLandingPage({ config }: { config: FrPageConfig }) {
  const canonical = `${SITE}/fr/${config.slug}`;
  ...
  const productJsonLd = {
    ...
    material: "Acétate italien Mazzucchelli 1849",
  ...
  const related = frPageOrder.filter((s) => s !== config.slug);
      ...
      <main style={{ background: colors.ink, color: colors.cream, minHeight: "100vh" }}>
        <header className="px-6 md:px-10 absolute top-0 left-0 right-0 z-20" style={{ background: "transparent" }}>
          <div className="max-w-6xl mx-auto flex items-center justify-between py-5">
            <Link to="/fr" aria-label="Woolet — accueil" className="inline-flex items-center">
        ...
        {related.length > 0 && (
          <section className="px-6 md:px-10" style={{ borderTop: `1px solid ${colors.line}` }}>
            <div className="max-w-5xl mx-auto py-20 grid sm:grid-cols-2 gap-5">
              {related.map((slug) => {
                const r = frPages[slug];
                return (
                  <Link key={slug} to={`/fr/${slug}`} className="block p-7 transition-colors hover:bg-[rgba(202,164,73,0.05)]" style={{ border: `1px solid ${colors.line}`, background: colors.inkSoft, textDecoration: "none", color: colors.cream }}>
                    ...
                    <div style={{ marginTop: 18, fontSize: 11, letterSpacing: "0.22em", textTransform: "uppercase", color: colors.gold }}>Lire la suite →</div>
                  </Link>
                );
              })}
            </div>
