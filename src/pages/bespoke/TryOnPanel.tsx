        ...
        {!isSignedIn ? (
          ...
          <>
            ...
            {result && <div className="mt-6 border border-cream/10 p-5">
              <div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-gold-light" aria-hidden /><div><h4 className="text-sm text-cream">{locale === "pl" ? "Zapisać zdjęcie na twarzy dla pracowni?" : "Save the on-face photo for the workshop?"}</h4><p className="mt-2 text-xs leading-relaxed text-cream-dim">{CONSENT_TEXT[locale]}</p></div></div>
              {!saved ? <>
                ...
                <button type="button" onClick={saveForWorkshop} disabled={!consent || saving} className="mt-3 min-h-[48px] w-full bg-gold text-background text-xs uppercase tracking-[0.18em] disabled:opacity-40">{saving ? (locale === "pl" ? "Zapisywanie…" : "Saving…") : (locale === "pl" ? "Zapisz zdjęcie na twarzy" : "Save the on-face photo")}</button>
                <p className="mt-3 text-[11px] leading-relaxed text-cream-dim">{locale === "pl" ? "Zapisane zdjęcie trafi do raportu PDF dla pracowni. Bez zgody żaden obraz nie jest przechowywany." : "The saved photo is included in the workshop PDF report. Without consent, neither image is stored."} <Link to={`/${locale}/privacy-policy#fit-scan`} className="underline">{locale === "pl" ? "Jak to przetwarzamy" : "How we handle it"}</Link></p>
              </> : <div className="mt-4"><p className="flex items-center gap-2 text-sm text-success"><Check size={16} /> {locale === "pl" ? "Zapisane — zdjęcie na twarzy trafi do raportu dla pracowni." : "Saved — the on-face photo will appear in the workshop report."}</p><button type="button" onClick={removeSaved} disabled={saving} className="mt-3 min-h-[44px] text-xs uppercase tracking-[0.18em] text-cream-dim underline">{locale === "pl" ? "Wycofaj zgodę i usuń zdjęcia" : "Withdraw consent and delete images"}</button></div>}
            </div>}
          </>
        )}
      </div>
