...
import { jaPageOrder, jaPages, type JaPageConfig } from "@/content/ja/landingPages";
...
export default function JaLandingPage({ config }: { config: JaPageConfig }) {
  const canonical = `${SITE}/ja/${config.slug}`;
  ...
  const productJsonLd = {
    ...
    material: "イタリア製アセテート (Mazzucchelli 1849)",
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "日本", item: `${SITE}/ja` },
      { "@type": "ListItem", position: 3, name: config.primaryKeyword, item: canonical },
    ],
  };

  const related = jaPageOrder.filter((s) => s !== config.slug);
      ...
      <main style={{ background: colors.ink, color: colors.cream, minHeight: "100vh" }}>
        ...
        >
          <div className="max-w-6xl mx-auto flex items-center justify-between py-5">
            <Link to="/ja" aria-label="Woolet — ホーム" className="inline-flex items-center">
        ...
        >
          <div className="max-w-6xl mx-auto pt-28 md:pt-32 pb-20 md:pb-28 grid md:grid-cols-[1.2fr_1fr] gap-12 items-center">
            ...
            <div className="hidden md:block">
              <img
                src="/og-image.png"
                alt={`Woolet ${config.primaryKeyword}`}
                loading="lazy"
                width={520}
                height={520}
            ...
            <div className="flex flex-col">
              {config.faqs.map((f, i) => (
                <details
                  ...
                  style={{ borderTop: i === 0 ? `1px solid ${colors.line}` : "none", borderBottom: `1px solid ${colors.line}` }}
                >
                  <summary
