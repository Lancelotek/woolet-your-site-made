    ...
    if (de) {
      const meta = base(
        ...
        lang,
        {
          ...
          noscriptHtml: `<h1>${escapeHtml(de.h1)}</h1><p>${escapeHtml(de.sub)}</p>${de.kurzeAntwort ? `<aside><p><strong>Kurze Antwort</strong></p><p>${escapeHtml(de.kurzeAntwort)}</p></aside>` : ""}<h2>${escapeHtml(de.problemTitle)}</h2><p>${escapeHtml(de.problemBody)}</p><h3>${escapeHtml(de.detailTitle)}</h3><p>${escapeHtml(de.detailBody)}</p>${de.contextualLink ? `<p>${escapeHtml(de.contextualLink.before)}<a href="/de/brillen-fuer-grosse-koepfe">${escapeHtml(de.contextualLink.label)}</a>${escapeHtml(de.contextualLink.after)}</p>` : ""}${de.widthTable ? `<h2>${escapeHtml(de.widthTable.title)}</h2><table><thead><tr><th>Gesichtsbreite (mm)</th><th>Kategorie</th><th>Empfohlene Frontbreite</th></tr></thead><tbody>${de.widthTable.rows.map((r) => `<tr>${r.map((c) => `<td>${escapeHtml(c)}</td>`).join("")}</tr>`).join("")}</tbody></table>` : ""}${(WIDTH_SLUGS as readonly string[]).includes(slug) ? `<h2>Andere Breiten</h2><ul>${WIDTH_SLUGS.filter((w) => w !== slug).map((w) => `<li><a href="/de/${w}">${escapeHtml(dePageTitles[w])}</a></li>`).join("")}</ul>` : ""}<h2>Passende Ratgeber</h2><ul>${de.related.map((relatedSlug) => `<li><a href="/de/${relatedSlug}">${escapeHtml(dePageTitles[relatedSlug])}</a></li>`).join("")}</ul>`,
        },
        { image: DEFAULT_OG, type: "website" },
        [
          breadcrumbJsonLd([
