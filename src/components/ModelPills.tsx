...
const ModelPills = (_props: ModelPillsProps = {}) => {
  ...
  const kickstarterHref = hrefFor("lp.kickstarter", lang);
  ...
  const modelCard = (
    ...
    <Link
      to={kickstarterHref}
      onClick={() =>
        pushGtmEvent("collection_cta_join_waitlist_click", {
          location: "collection_model_pill",
          dest: "lp_kickstarter",
          sku,
        })
      }
      className="flex-1 border p-4 flex flex-col gap-1 transition-colors hover:border-primary/20 relative overflow-hidden group no-underline"
    ...
    >
      ...
      <div className="font-display text-woolet-white" style={{ fontSize: "1.25rem" }}>{name}</div>
      <div className="text-cream-dim" style={{ fontSize: "0.78rem" }}>{t(lang, specsKey)}</div>
      <div
        className="mt-2 inline-flex items-center justify-center uppercase tracking-[0.22em] transition-colors self-start"
        style={{
